...
- The model uses long no-op shell waits such as `Start-Sleep -Seconds ...; Write-Output "ok"` instead of relying on extension retry or reporting retry exhaustion.
...
| Shell no-op wait guard | `KIRO_getBlockedNoopWaitReason`, `Blocked no-op shell wait command` | Rejects long `Start-Sleep` plus `Write-Output "ok"` / `"retry"` no-op backoff |
...
## 10. Shell No-Op Wait Guard

### 10.1 Original Issue

After high-load, throttling, or sub-agent failures surface back to the agent, the model may call `execute_pwsh` only to wait, for example:

```powershell
Start-Sleep -Seconds 360; Write-Output "ok"
```

This command is not hardcoded by the extension. It is generated by the model when it uses the shell as a retry/backoff timer. The practical effect is that the task remains running for a long time without performing useful checks or edits.
...
Both shell tool execution paths now run a pre-execution check. The following patterns are rejected:
...
The guard only blocks no-op waits with `N >= 60`. The returned tool error tells the model not to use shell waiting for model high load, throttling, or network errors. It should let the extension stream retry handle transient failures; if retries are exhausted, it should report that state to the user.

### 10.3 Allowed Cases

The guard does not block:

- Short waits such as `Start-Sleep -Seconds 2`.
- Waits followed by real diagnostics, such as `Start-Sleep -Seconds 2; Get-ChildItem ...`.
- Real long-running commands such as dev servers or watchers, which are still handled by the existing long-running command rules.

The intent is to block only long “wait then print ok/retry/done” shell backoff, without breaking normal diagnostics.

## 11. Empty FileDecoration Fix
...
## 12. `getWorkspaceFiles()` Deduplication
...
## 14. Installation Verification Flow
...
### 14.4 Runtime Observation Points
...
### 15.1 `Too many requests` Still Appears
...
### 15.2 Usage-Limit Errors Still Appear
...
### 15.3 `Failed to invoke Spec Task Execution` Still Appears
...
### 15.5 Long `Start-Sleep` Still Appears

This no-op wait should be rejected:

```powershell
Start-Sleep -Seconds 360; Write-Output "ok"
```

Check whether the installed bundle contains:

```powershell
Select-String -Path $target -Pattern "KIRO_getBlockedNoopWaitReason","Blocked no-op shell wait command"
```

If it does not, the installed bundle is not the latest patched version or Kiro has overwritten it during an update.

This command is still allowed because it performs a real check after a short wait:

```powershell
Start-Sleep -Seconds 2; Get-ChildItem ...
```

### 15.6 Patch Disappears After Kiro Update
